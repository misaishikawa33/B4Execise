# 全方位画像を生成，画角を画像サイズから計算
# コマンドライン引数として，画像名，画角（水平方向，垂直方向），出力画像サイズ（高さ，幅）を入力
...
def main():
    ...
    new_img_theta = int(args[2])
    new_img_phi = int(args[3])
    new_img_size = np.array([args[4], args[5]]).astype(int)
    # print(type(theta))
    # print(type(output_img_size[0]))
    ...
    # 透視投影画像を生成
    new_img = omnidirectional_img(img, new_img_theta, new_img_phi, new_img_size)
...
# 透視投影画像を生成
def omnidirectional_img(img, new_img_theta, new_img_phi, new_img_size):

    #画素間の長さを計算
    angle_degree = math.radians(new_img_theta/2)
    delta_x = 2 * math.tan(angle_degree) / new_img_size[1]

    angle_degree = math.radians(new_img_phi/2)
