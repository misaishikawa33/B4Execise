# 視点回転を含めた投資投影画像の生成
# コマンドライン引数として，画像名，視線角度（水平，垂直，光軸），（angleを入力してから）画角（水平，垂直），（sizeを入力してから）画像サイズ（高さ，幅）
...
def main():
    ...
    theta_eye, phi_eye, psi_eye = int(args[2]), int(args[3]), int(args[4]) #視線角度
    ...
    # 透視投影画像を生成
    new_img = perspective_projection_img(img, theta_eye, phi_eye, psi_eye, new_img_theta, new_img_phi, new_img_size)
...
# 透視投影画像を生成
def perspective_projection_img(img, theta_eye, phi_eye, psi_eye, new_img_theta = 0, new_img_phi = 0, new_img_size = np.zeros(2)):
    ...
    # 回転行列の計算
    # 視点角度をラジアンに変換
    theta_eye, phi_eye, psi_eye = math.radians(theta_eye), math.radians(phi_eye), math.radians(psi_eye)
